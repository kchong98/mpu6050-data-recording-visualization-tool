...
class PlotRecAccGyro():
	def __init__(self, parent):
		...
		# Checkbuttons - Displacement
		for i,j in enumerate(['Disp_x', 'Disp_y', 'Disp_z']):
			self.var_disp = IntVar()
			self.disp_chkbtn = Checkbutton(self.labelframe_comp, text=j, variable=self.var_disp, command=self.components_chk_func, padx=5, pady=5)
			self.disp_chkbtn.grid(row=i, column=2)
			self.accgyro_chkbtn_state.append(self.var_disp)
			self.chkbtn_list.append(self.disp_chkbtn)
	...
	def all_chk_func(self):
		if [i.get() for i in self.accgyro_chkbtn_state] != ([1]*9):
	...
	def plot_start_stop(self, btn_name):
		if self.plot_chkbtn['text'] == 'Start Plotting':
			...
			if (1 in acc_n_plots) and (1 in gyro_n_plots) and (1 in disp_n_plots):
				...
				fig, ax = plt.subplots(1,1, figsize = (10,4), projection='3d')
				if 1 in acc_n_plots:
					acc_plot_list = [ax.plot3D(time_values, acc_values[0], zeros, label = acc_legends[i])[0] if j==1 else 0 for i,j in enumerate(acc_n_plots)]
					ax.set_title('Acceleration')
					ax.set_xlabel('Readings')
					ax.set_ylabel('Acceleration [m/s^2]')
					ax.set_ylim([-30,30])
					ax.grid()
				elif 1 in gyro_n_plots:
					gyro_plot_list = [ax.plot3D(time_values, gyro_values[0], zeros, label = gyro_legends[i])[0] if j==1 else 0 for i,j in enumerate(gyro_n_plots)]
			...
			while self.plot_trigger:
				# Funtion to calculation displacement from acceleration data
				...
				dispX = getdisplacement(accel_data['x'])
				dispY = getdisplacement(accel_data['y'])
				dispZ = getdisplacement(accel_data['z'])
				current_disp = [dispX, dispY, dispZ]
				for i, j in enumerate(disp_n_plots):
					if j == 1:
						disp_values[i].append(current_disp[i])
						zeros.append(0)
						del disp_values[i][0]
						disp_plot_list[i].set_ydata(disp_values[i])
