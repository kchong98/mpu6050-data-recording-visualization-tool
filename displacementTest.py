	...
	def plot_start_stop(self, btn_name):
		if self.plot_chkbtn['text'] == 'Start Plotting':
			...
			if (1 in acc_n_plots) and (1 in gyro_n_plots):
				...
				for i, (title, y_axis_label) in enumerate([('Acceleration', 'Acceleration [m/s^2]'), ('Displacement', 'Length [m]')]):
				...
				disp_plot_list = [ax[1].plot(time_values, disp_values[0], label = disp_legends[i])[0] if j==1 else 0 for i,j in enumerate(gyro_n_plots)]
				...
				if 1 in acc_n_plots:
					...
					disp_plot_list = [ax.plot(time_values, disp_values[0], label = disp_legends[i])[0] if j==1 else 0 for i,j in enumerate(gyro_n_plots)]
